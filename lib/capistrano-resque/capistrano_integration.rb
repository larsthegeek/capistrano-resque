              ...
              workers.each_pair do |queue, number_of_workers|
                ...
                number_of_workers.times do
                  ...
                  run("cd #{current_path} && RAILS_ENV=#{rails_env} QUEUE=\"#{queue}\" \
                   PIDFILE=#{pid} BACKGROUND=yes #{rails_env == 'production' ? "VERBOSE=1" : "VVERBOSE=1"} #{fetch(:bundle_cmd, "bundle")} exec rake environment resque:work >> #{shared_path}/log/resque.log 2>&1 &", 
                      :roles => role)
                  worker_id += 1
                end
              end
